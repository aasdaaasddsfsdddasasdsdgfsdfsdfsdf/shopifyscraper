...
const REVIEWERS = ['Selçuk', 'Gürkan', 'Sefa', 'Efkan', 'Mert Tufan', 'Furkan', 'Simay', 'Talha'];
...
function App() {
  ...
  const [filterPazar, setFilterPazar] = useState('');
  const [filterListedurum, setFilterListedurum] = useState('');
  const [filterInceleyen, setFilterInceleyen] = useState('');
  ...
  const loadProducts = useCallback(async (currency: string, page: number) => {
    ...
    try {
      ...
      if (filterListedurum === 'yes') {
        query = query.eq('listedurum', true);
      } else if (filterListedurum === 'no') {
        query = query.eq('listedurum', false);
      } else if (filterListedurum === 'empty') {
        query = query.is('listedurum', null);
      }
      ...
      if (filterInceleyen) {
        query = query.eq('inceleyen', filterInceleyen);
      }
    ...
    setIsLoading(false);
  }, [filterNiche, filterDomain, filterTitle, filterCiroMin, filterCiroMax, filterPazar, filterListedurum, filterInceleyen]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [activeTab, filterNiche, filterDomain, filterTitle, filterCiroMin, filterCiroMax, filterPazar, filterListedurum, filterInceleyen]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <div className="max-w-[1920px] mx-auto px-6 py-8">
        <div className="mb-8 flex flex-col items-center">
          ...
          <div className="w-full max-w-6xl bg-gray-800 border border-gray-700 rounded-xl p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Pazar Ara</label>
                <input
                  type="text"
                  placeholder="Örn: DE, FR"
                  value={filterPazar}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Listeleme Durumu</label>
                <select
                  value={filterListedurum}
                  onChange={(e) => setFilterListedurum(e.target.value)}
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:border-blue-500 focus:outline-none transition"
                >
                  <option value="">Tümü</option>
                  <option value="yes">Evet (Listelendi)</option>
                  <option value="no">Hayır</option>
                  <option value="empty">Boş (Atanmadı)</option>
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">İnceleyen</label>
                <select
                  value={filterInceleyen}
                  onChange={(e) => setFilterInceleyen(e.target.value)}
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:border-blue-500 focus:outline-none transition"
                >
                  <option value="">Tümü</option>
